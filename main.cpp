...
void OnCreate(HWND hw) {
	HWND bOne = CreateWindowA("BUTTON", "one", WS_CHILD | WS_VISIBLE,
		80, 20, 120, 60,
		hw, (HMENU) id_button1, (HINSTANCE)GetWindowLong(hw, GWL_HINSTANCE), NULL);

	HWND bTwo = CreateWindowA("BUTTON", "two", WS_CHILD | WS_VISIBLE,
		80, 85, 120, 60,
		hw, (HMENU) id_button2, (HINSTANCE)GetWindowLong(bOne, GWL_HWNDPARENT), NULL);
}

void OnCommand(HWND hw, int id) {
	if(id==id_button1) MessageBox(hw, "one", "NWP", MB_OK | MB_ICONWARNING);
	if(id==id_button2) MessageBox(hw, "two", "NWP", MB_OK | MB_ICONWARNING);
}
